...
| --- | --- | --- | --- |
| **Python** | 3.12+ | The app is written in Python | Courier-side view of Python — affects how the courier loads, reads, or delivers the parcels |
...
| **Git** | 2.40+ | Version control | Courier-side view of Git — affects how the courier loads, reads, or delivers the parcels |
| **AWS CLI** | 2.x | Connect to AWS services | AWS-side depot yard — AWS CLI: 2.x · Connect to AWS services |
...
| **Docker** | 24+ | Build container images (optional, for deployment) | The shipping crate that lets the same courier-and-depot run on any farm — laptop, AWS, Azure, all identical. |
| **Terraform** | 1.5+ | Deploy infrastructure (optional, for deployment) | Depot blueprints — `terraform apply` and a fresh barn appears in AWS or Azure exactly as drawn. |
| **VS Code** or **PyCharm** | Latest | IDE with debugger | Your saddle and reins — IDE plus debugger so you can stop the courier mid-trip and inspect what it's carrying. |
...
- Latest depot version with full type hint support
...
| --- | --- | --- | --- |
| **RAM** | 8 GB | 16 GB | Courier-side view of RAM — affects how the courier loads, reads, or delivers the parcels |
| **Disk** | 5 GB (for models) | 10 GB | Blank shipping manifest — Disk: 5 GB (for models) · 10 GB |
| **GPU** | Not required (CPU works) | NVIDIA GPU (faster inference) | Courier-side view of GPU — affects how the courier loads, reads, or delivers the parcels |
...
| Error | Cause | Fix | 🚚 Courier |
| --- | --- | --- | --- |
| `DeploymentNotFound` (404) | Embedding model not deployed on the same Azure OpenAI resource as the LLM | Deploy `text-embedding-3-small` on the **same** resource as `gpt-4o`. Both must share one `AZURE_OPENAI_ENDPOINT`. | Courier can't find the embedding stall — it's at a different Azure depot than the writing stall |
| `content_filter` (400) | Azure blocking prompt injection test prompts | Expected in Phase 4 guardrails labs. Azure's built-in content filter blocks jailbreak attempts. | Shipping manifest pinned to the saddle — content_filter (400): Azure blocking prompt injection test prompts · Expected in Phase 4 guardrails labs. Azure's built-in content |
| `RAG chain not initialized` | `.env` still has `<your-resource>` placeholder values | Check all `AZURE_*` values in `.env` — replace every `<your-resource>` with real values. | Label on the original mail item the parcel was sliced from |
| `Port already in use` | Old server process still running | Run `pkill -f uvicorn` then retry. | Depot manager — receives requests at the front door and dispatches the courier |
| `Region not supported` | Model not available in chosen Azure region | Use **East US**, **East US 2**, or **Sweden Central** for both `gpt-4o` and `text-embedding-3-small`. | The chosen Azure region doesn't depot that model — relocate the courier to a region that does |
...
| Test file | Tests | What it verifies | 🚚 Courier |
| --- | --- | --- | --- |
| `test_chat.py` | 8 | API endpoints: health, chat validation, document list/delete | Eight tests poking the courier's depot door — health, chat validation, and document list/delete endpoints. |
| `test_evaluation.py` | 12 | RAG evaluation: retrieval scoring, faithfulness, relevance, golden dataset | How confidently the warehouse says 'this parcel matches' — higher = closer GPS hit |
| `test_ingestion.py` | 8 | Document parsing (TXT/MD/CSV), text chunking, overlap, edge cases | Eight tests covering how the post office slices documents into overlapping parcel pockets. |
...
| Score | What it means | Healthy range | 🚚 Courier |
|---|---|---| --- |
| `retrieval` | Did vector search find relevant chunks? | ≥ 0.7 | Did the courier's vector search actually grab the right parcel pockets? Pass mark is 0.7. |
| `faithfulness` | Does the answer stick to the context? | ≥ 0.8 | Did the courier stick to the parcels it was carrying, or invent stuff on the way? |
| `answer_relevance` | Does the answer address the question? | ≥ 0.6 | Routing tag on the parcel — answer_relevance: Does the answer address the question? · ≥ 0.6 |
| `overall` | Weighted average (retrieval 30% + faithfulness 40% + relevance 30%) | ≥ 0.7 | How confidently the warehouse says 'this parcel matches' — higher = closer GPS hit |
| `passed` | Overall ≥ 0.7 | `true` | Courier's report card — share of test deliveries that scored above the bar |
...
| Phase | What it teaches | AI engineering skill | 🚚 Courier |
| --- | --- | --- | --- |
| **Phase 1** — Foundation (Labs 1–2) | Retrieval quality, faithfulness scoring, hallucination detection, top_k tuning | You learn to **measure** AI system quality — the foundation of every production AI system. Without evaluation, you're deploying blind. | Foundation labs that measure whether the courier is fetching the right parcel pockets without hallucinating. |
| **Phase 2** — Bridge (Labs 3–5) | Business metrics, prompt injection guardrails, AI observability dashboards | You learn to **translate** technical scores into business language, **protect** against adversarial inputs, and **monitor** AI systems in production. | Note the courier carries — Phase 2 — Bridge (Labs 3–5): Business metrics, prompt injection guardrails, AI observability dashboards · You learn to translate technical |
| **Phase 3** — Production (Labs 6–8) | Data flywheel, RLHF feedback loops, infrastructure scaling | You learn the **continuous improvement loop** that separates production AI from demos — detect bad answers, fix them, lock the fix with golden datasets, repeat. | Graduating from one-off rides to a full fleet — feedback loops, golden datasets, robot dispatch clerk keeping it tidy. |
...
| Environment | Expected time | Why | 🚚 Courier |
| --- | --- | --- | --- |
| **Local (Ollama CPU)** | ~20–25 min | Each query takes 5–60 s on CPU | CPU-fed courier trots slowly — every delivery takes up to a full minute on fuel-power alone |
| **Local (Ollama GPU)** | ~10–15 min | NVIDIA GPU speeds up inference ~2× | GPU-fed courier runs roughly twice as fast as the CPU version on the same routes |
| **AWS (Bedrock/Claude)** | ~3–5 min | Cloud LLMs respond in 1–5 s per query | Cloud courier on Bedrock writes answers in seconds, dramatically faster than the local version |
| **Azure (GPT-4o)** | ~3–5 min | Cloud LLMs respond in 1–5 s per query | Cloud courier on Azure GPT-4o writes answers in seconds, similar speed to the AWS path |
...
| Phase | Experiments | API calls | 🚚 Courier |
| --- | --- | --- | --- |
...
| **Phase 4** — Advanced RAG | 9a (injection ×3), 9b (PII ×3), 9c, 10a (×3), 10b (×3), 11a-c (×8), 12a-b, 13a-d (×6) | 7 chat + 22 evaluate + 1 upload-batch | Courier's report card — automated grading of test deliveries |
| **Phase 5** — Observability | 14a (query stats), 14b (failures), 15a (metrics), 16a (golden suite) | 1 stats + 1 failures + 1 metrics + 1 suite call | Stopwatch on the courier's harness — Phase 5 — Observability: 14a (query stats), 14b (failures), 15a (metrics), 16a (golden suite) · 1 stats + 1 |
| **Thinking exercises** | 3b, 4b, 5c, 7a, 7b, 8a, 8b | 0 (skipped — these are design exercises) | Courier-side view of Thinking exercises — affects how the courier loads, reads, or delivers the parcels |
| **Total** | 50 API experiments + 7 thinking | 47 API calls + 7 skipped | Depot's monthly feed bill — Total: 50 API experiments + 7 thinking · 47 API calls + 7 skipped |
...
| Endpoint | Method | What it does | 🚚 Courier |
| --- | --- | --- | --- |
| `/api/health` | GET | Checks if the RAG chain is initialised. No AI calls. | Depot's front door — the URL customers use to drop off a question |
| `/api/chat` | POST | Send a question, get an AI answer with sources and token usage (the full RAG pipeline). | Send a question, get the courier's answer plus cited parcel pockets and a fuel tally for this delivery. |
| `/api/documents/upload` | POST | Upload a single document — the app chunks, embeds, and stores it. | Upload one document and the post office immediately chunks, embeds, and files its parcel pockets. |
| `/api/documents/upload-batch` | POST | Upload multiple documents in one request. | Where parcels are dropped at the depot — /api/documents/upload-batch: POST · Upload multiple documents in one request. |
| `/api/documents` | GET | List all uploaded documents and their chunk counts. | Lists every uploaded document along with how many parcel pockets each was sliced into. |
...
| `/api/evaluate` | POST | Run a question through the RAG pipeline AND score the answer quality (RAGAS metrics). | How confidently the warehouse says 'this parcel matches' — higher = closer GPS hit |
| `/api/evaluate/suite` | POST | Run the full golden dataset — like `dbt test` for your AI system. | Sends the courier on all 25 standard test deliveries and returns one combined report card |
| `/api/queries/stats` | GET | Aggregate pass rate and failure breakdown from query logs. | Depot's front door — /api/queries/stats: GET · Aggregate pass rate and failure breakdown from query logs. |
| `/api/queries/failures` | GET | Recent failed queries with failure categories (bad_retrieval, hallucination, etc.). | Recent deliveries where the courier strayed — categorised as bad retrieval, hallucination, and friends. |
...
| Component | Why it just works | 🚚 Courier |
| --- | --- | --- |
| `POST /api/chat` | Asks questions against whatever is in the vector store | Where parcels are dropped at the depot — POST /api/chat: Asks questions against whatever is in the vector store |
| `POST /api/documents/upload` | Accepts any PDF, TXT, MD, CSV, or DOCX | Entry gate to the depot — POST /api/documents/upload: Accepts any PDF, TXT, MD, CSV, or DOCX |
| `GET /api/documents` | Lists whatever you've uploaded | Where parcels are dropped at the depot — GET /api/documents: Lists whatever you've uploaded |
...
| `src/rag/` (chain, ingestion, prompts) | Generic RAG pipeline — not tied to any document | Shipping manifest pinned to the saddle — src/rag/ (chain, ingestion, prompts): Generic RAG pipeline — not tied to any document |
| `src/llm/`, `src/vectorstore/`, `src/storage/` | Provider implementations — fully generic | Swappable courier, warehouse, and depot implementations chosen at runtime by config |
| `src/config.py`, `src/main.py` | App configuration — no document references | Courier-side view of src/config.py`, `src/main.py — affects how the courier loads, reads, or delivers the parcels |
...
| Section | What to change | 🚚 Courier |
| --- | --- | --- |
...
| `golden_dataset` | Rewrite the 25 Q&A test cases to match your document (questions, expected keywords, context chunks) | Rewrite the 25 standard test deliveries — questions, expected keywords, and reference parcel pockets — for your docs. |
| `lab_questions.phase1` | Baseline + retrieval questions about your document | Courier grabs the nearest parcels from the GPS warehouse before writing the answer |
...
| `lab_questions.phase3` | Gap question + a "gap document" your knowledge base does NOT have | Courier-side view of lab_questions.phase3 — affects how the courier loads, reads, or delivers the parcels |
...
| File | What to change | Impact if skipped | 🚚 Courier |
| --- | --- | --- | --- |
| `tests/test_evaluation.py` | Mock fixtures reference hardcoded golden dataset entries (questions, answers, chunks) | Unit tests will fail | Mock fixtures hardcode questions, answers, and parcel pockets; swapping the dataset will break unit tests. |
...
| `scripts/run_all_labs.py` (Phase 4 Labs 11–13) | A few hardcoded structural questions remain — mostly generic but review if your doc is very different | May produce odd results | Practice run — readers play dispatch clerk and put the courier through its paces |

### Files you SHOULD update (but won't break anything)

| File | What's there | Impact if skipped | 🚚 Courier |
| --- | --- | --- | --- |
| `docs/hands-on-labs/hands-on-labs-phase-1.md` | Lab walkthroughs reference refund policy questions | Labs won't match your results | Depot keys — only authorised callers may ask the courier to deliver |
| `docs/hands-on-labs/hands-on-labs-phase-2.md` | Dashboard questions reference refunds | Cosmetic only | Dashboard the depot owner watches — flags slow or failing courier trips |
| `docs/hands-on-labs/hands-on-labs-phase-3.md` | Data flywheel uses remote-work-policy as gap doc | Scenario won't make sense | Depot keys — only authorised callers may ask the courier to deliver |
...
| `src/api/models.py` (field examples) | `example="What is the refund policy?"` | Swagger UI placeholder | Depot's front door — src/api/models.py (field examples): example="What is the refund policy?" · Swagger UI placeholder |
